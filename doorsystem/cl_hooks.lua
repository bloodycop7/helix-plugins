   ...
   for k, v in pairs(ents.FindInSphere(ply:GetPos(), 150)) do
       for a, b in pairs(PLUGIN.doors) do
           if ( classes[v:GetClass()] ) and ( tostring(a) == v:MapCreationID() ) then
               ...
               draw.DrawText(b["name"], "CombineFont24", pos.x, pos.y, ColorAlpha(color_white, 255), TEXT_ALIGN_CENTER)

               if ( b["access"] ) then
                   for c, d in pairs(PLUGIN.access) do
                       if ( b["access"] == c ) then
                           draw.DrawText(d["name"], "CombineFont26", pos.x, pos.y - 30, ColorAlpha(d["color"], 255), TEXT_ALIGN_CENTER)
                       end
                   end
               end
           end
